...
### После каждого пуша (CI)

- **После каждого `git push` ОБЯЗАТЕЛЬНО проверять GitHub Actions (CI)**
- Команда для проверки: `gh run list --limit 1`
- **Если CI красный** — **не продолжать работу**, сначала исправить ошибку
- **Если CI зелёный** — можно продолжать к следующей задаче/фазе
- Не переходить к следующей задаче пока CI не пройдёт успешно
...
- [ ] KANBAN.md актуализирован (задача отмечена [x])
- [ ] Задача отмечена `[x]` в KANBAN.md
- [ ] CI зелёный после пуша (`gh run list --limit 1`)